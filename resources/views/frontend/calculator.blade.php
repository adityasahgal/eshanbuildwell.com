...
$meta_description = "Get an instant preliminary construction cost estimate for your residential or commercial project. Enter your details and calculate in real-time.";
...
<style>
    ...
    .estimate-pro .input-group { display: flex; flex-direction: column; }
    ...
    .estimate-pro .input-group input,
    .estimate-pro .input-group select {
        padding: 10px 12px;
    ...
    .estimate-pro .input-group input:focus,
    .estimate-pro .input-group select:focus { border-color: var(--orange); box-shadow: 0 0 0 3px rgba(255,107,0,.12); }
    .estimate-pro .input-group input[readonly] { background: #f3f4f6; color: #6b7280; cursor: not-allowed; }

    /* ---- Summary table ---- */
    .estimate-pro .summary-table { width: 100%; border-collapse: collapse; }
    .estimate-pro .summary-table td { padding: 11px 0; border-bottom: 1px solid #e5e7eb; font-size: .95rem; color: #374151; }
    .estimate-pro .summary-table td:last-child { text-align: right; font-weight: 600; color: #111827; }
    .estimate-pro .summary-table .total-row td { font-size: 1.2rem; font-weight: 700; color: var(--orange); border-bottom: none; padding-top: 16px; }
    .estimate-pro .gst-note { font-size: .85rem; color: #ef4444; text-align: right; margin-top: 8px; font-weight: 500; }
    .estimate-pro .note-text { font-size: .85rem; color: #6b7280; line-height: 1.5; margin-top: 14px; }
    ...
    .estimate-pro .premium-text h3 { margin: 0 0 8px 0; font-size: 1.25rem; color: #fff; }
    .estimate-pro .premium-text p  { margin: 0 0 8px 0; color: #cbd5e1; font-size: .95rem; }
    ...
    .estimate-pro .premium-price { font-size: 1.1rem; font-weight: 600; color: #fbbf24; }

    /* ===========================================================
       STEP 1 – User Info Card
    =========================================================== */
    .user-info-wrapper {
        max-width: 560px;
    ...
    /* ===========================================================
       Step badges
    =========================================================== */
    .step-tracker { display: flex; align-items: center; justify-content: center; gap: 0; margin-bottom: 40px; }
    .step-node {
    ...
    .step-circle {
        ...
        display: flex; align-items: center; justify-content: center;
        font-weight: 700; font-size: .9rem; color: #9ca3af;
    ...
    /* User badge in calc header */
    .user-badge {
        background: #f0fdf4; border: 1px solid #bbf7d0;
        color: #15803d; padding: 7px 14px; border-radius: 20px;
...
<section class="py-5 bg-light estimate-pro">
    <div class="container">
        ...
        <div id="step1">
            <div class="user-info-wrapper">
                <div class="user-info-card">
                    ...
                    <button class="btn-proceed" id="btnProceed" onclick="proceedToCalculator()">
        ...
        {{-- ===================================================
             STEP 2 – Calculator (hidden until step 1 done)
        ==================================================== --}}
        <div id="step2" style="display:none;">
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-7">
                    <div class="card h-100">
                        <div class="card-title"><span>📐</span> Project Inputs</div>
                        <div class="form-grid">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" id="plotArea" readonly value="0">
                            </div>
                            <div class="input-group">
                                <label>Built-up Area per Floor (sq.ft)</label>
                                <input type="number" id="builtupPerFloor" value="0" min="0" step="100">
                            </div>
                            <div class="input-group">
                ...
                <div class="col-lg-5">
                    <div class="card h-100">
                        <div class="card-title"><span>💰</span> Cost Estimate Summary</div>
            ...
            <div class="premium-box mt-4">
                ...
                <div class="price-block mb-3 mb-md-0">
                    <div class="premium-price">₹4/sq.ft <span style="font-size:.9rem;color:#cbd5e1;font-weight:400;">(incl. GST)</span></div>
                    <div style="font-weight:700;margin-top:5px;font-size:1.1rem;">Estimation Charges: ₹<span id="estimationCharge">0</span></div>
                </div>
                <button class="btn-premium" id="contactBtn">📞 Request Professional Estimate</button>
            </div>
            <div class="note-text text-center mt-2 mb-4">
...
<script>
    // ----------------------------------------------------------------
    // Rate tables – injected from DB via PHP (dynamic)
    // ----------------------------------------------------------------
    ...
    function proceedToCalculator() {
        ...
        if (name.length < 2) {
            ...
            nameInp.classList.remove('error-input');
        }
        ...
        if (!emailRx.test(email)) {
            ...
            emailInp.classList.remove('error-input');
        }
        ...
        if (!phoneRx.test(phone)) {
            ...
            phoneInp.classList.add('error-input');
            valid = false;
        } else {
            errPhone.style.display = 'none';
            phoneInp.classList.remove('error-input');
        }

        if (!valid) return;

        // Update step tracker
        document.getElementById('sc1').classList.remove('active');
        document.getElementById('sc1').classList.add('done');
        document.getElementById('sc1').innerHTML = '<i class="fas fa-check" style="font-size:.8rem"></i>';
    ...
    // Allow Enter key on step-1 inputs
    ...
    // ----------------------------------------------------------------
    // STEP 2 – Calculator logic
    // ----------------------------------------------------------------
    function calculateAll() {
        const plotLength      = +document.getElementById('plotLength').value      || 0;
        const plotWidth       = +document.getElementById('plotWidth').value       || 0;
        const builtupPerFloor = +document.getElementById('builtupPerFloor').value || 0;
        const totalFloors     = +document.getElementById('totalFloors').value     || 1;

        const plotArea = plotLength * plotWidth;
        document.getElementById('plotArea').value = plotArea.toFixed(0);

        const safeBuiltup = (builtupPerFloor > plotArea && plotArea > 0) ? plotArea : builtupPerFloor;
        const totalBuiltup = safeBuiltup * totalFloors;
        document.getElementById('totalBuiltup').value = totalBuiltup.toFixed(0);
        document.getElementById('premiumArea').textContent = totalBuiltup.toFixed(0);
        ...
        const structureCost  = totalBuiltup * structureRates[stQ];
        ...
        document.getElementById('estimationCharge').textContent  = (totalBuiltup * 4).toLocaleString('en-IN');
    }
    ...
    ['plotLength','plotWidth','builtupPerFloor','totalFloors',
     ...
     'externalDev','location','contingency'].forEach(id => {
    ...
    document.getElementById('contactBtn').addEventListener('click', () => {
        ...
        alert(`📞 Thank you ${name}!\nOur expert team will contact you at ${phone} shortly with a detailed BOQ & accurate estimation for ${area} sq.ft.`);
    });
</script>
@endsection