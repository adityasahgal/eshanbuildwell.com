...
.bim-hero {
    ...
    background:
        ...
        url('{{ $content && $content->hero_image ? asset($content->hero_image) : 'https://images.unsplash.com/photo-1486325212027-8081e485255e?w=1400&q=80' }}') center/cover no-repeat;
    color: #fff;
}
...
.bim-cert-topbar .cb span:nth-child(1) {
    width: 16px;
}

.bim-cert-topbar .cb span:nth-child(2) {
    width: 10px;
}

.bim-cert-topbar .cb span:nth-child(3) {
    width: 16px;
}
...
.bim-cert-qr {
    ...
    flex-shrink: 0;
    background:
        repeating-conic-gradient(#222 0% 25%, #fff 0% 50%) 0 0/8px 8px;
    border-radius: 5px;
            ...
            <section class="bim-hero">
                <div class="bim-hero-grid">
                    <div>
                        ...
                        <h1>{!! $content->hero_title ?? 'BIM (Revit)<br>Professional Training' !!}</h1>
                        <p>
                            {{ $content->hero_description ?? 'Learn complete BIM workflow from 3D design to 4D planning and 5D costing with live project training using Autodesk Revit, Navisworks, and DiRoots. The program is designed for civil engineers, architects, and working professionals who want practical, industry-ready skills.' }}
                        </p>
                    ...
                    <div class="bim-hero-panel">
                        <div class="bim-hero-stats">
                            <div class="bim-stat">
                                <strong>{{ $content->hero_fee ?? '₹4,999/-' }}</strong>
                                <span>Course Fee</span>
                            </div>
                            <div class="bim-stat">
                                <strong>{{ $content->hero_duration ?? '3 Months' }}</strong>
                                <span>Duration</span>
                            </div>
                            <div class="bim-stat">
                                <strong style="font-size: 1.4rem;">{{ $content->hero_timing ?? 'Tues/Thur/Sat' }}</strong>
                                <span>6:30 PM - 8:00 PM</span>
                            </div>
                            <div class="bim-stat">
                                <strong style="font-size: 1.4rem;">{{ $content->hero_batch_start ?? '28th April' }}</strong>
            ...
            <div class="bim-body">
                ...
                <div class="bim-about-grid">
                    <div class="bim-highlight" style="margin-bottom:0;">
                        <h3>{{ $content->about_title ?? 'About the Training Program' }}</h3>
                        <p>
                            {{ $content->about_description_1 ?? 'This BIM training program is built around real project-based learning. You will understand how to develop building models, coordinate planning, generate quantity takeoff, and support costing workflows using the same digital tools used in modern construction execution.' }}
                        </p>
                        <p class="mt-3" style="margin-bottom:0; color:var(--text);">
                            {{ $content->about_description_2 ?? 'The course covers complete workflow from design (3D) to planning (4D) and costing (5D) — giving you end-to-end practical BIM skills that are industry-ready from day one.' }}
                        </p>
                    </div>
                    <div class="bim-about-photo">
                        <img src="{{ asset($content->about_image ?? 'assets/images/Project4.jpg') }}"
                            alt="Live BIM construction project training" loading="lazy" />
                    </div>
                </div>
                ...
                <div class="bim-section-head mt-4">
                    <div>
                        <h2>{{ $content->learn_section_title ?? 'What You Will Learn' }}</h2>
                    </div>
                    <p>{{ $content->learn_section_desc ?? 'Structured modules focused on architecture, structure, planning, and costing so learners can move from software basics to complete BIM workflow understanding.' }}</p>
                </div>
                    ...
                    <div>
                        <div class="bim-tool-card">
                            <div class="bim-tool-title">
                                ...
                                <div>
                                    <h3>{{ $content->tools_title ?? 'Tools & Software' }}</h3>
                                    <p>{{ $content->tools_desc ?? 'Industry tools covered during the BIM workflow training.' }}</p>
                                </div>
                            </div>
                            ...
                            <div class="row g-4">
                                <div class="col-md-6">
                                    ...
                                    <ul class="bim-bullets">
                                        @if($content && is_array($content->revit_desc_list))
                                            @foreach($content->revit_desc_list as $item)
                                    ...
                                    <div class="bim-trainer-box">
                                        <div class="bim-trainer-photo-row">
                                            <div class="bim-trainer-photo">
                                                <img src="{{ asset($content->trainer_image ?? 'assets/images/ceo.jpeg') }}"
                                                    alt="{{ $content->trainer_name ?? 'Er. Gagandeep Rajput' }} – Trainer, Eshan Buildwell"
                                                    loading="lazy" />
                                            </div>
                                            <div>
                                                <strong>{{ $content->trainer_name ?? 'Er. Gagandeep Rajput' }}</strong>
                                                <div class="sub">{{ $content->trainer_role ?? 'Construction & BIM Specialist' }}</div>
                                            </div>
                                        </div>
                                        <ul class="bim-bullets mt-1">
                                            @if($content && is_array($content->trainer_bullets))
                                                @foreach($content->trainer_bullets as $bullet)
                                                    <li><i class="bi bi-dot"></i> {{ $bullet }}</li>
                                                @endforeach
                                            @endif
                                        </ul>
                                    </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <ul class="bim-bullets">
                                        @if($content && is_array($content->other_tools_list))
                                            @foreach($content->other_tools_list as $tool)
                                                <li><i class="bi bi-check-lg"></i> {{ $tool }}</li>
                                            @endforeach
                                        @endif
                                    </ul>
                                </div>
                        ...
                        <div class="bim-feature-ribbon" style="margin-top: 0;">
                            @if($content && is_array($content->program_outcomes))
                                @foreach($content->program_outcomes as $outcome)
                                    <span class="bim-feature-pill"><i class="bi {{ $outcome['icon'] ?? 'bi-check-circle-fill' }}"></i> {{ $outcome['text'] ?? '' }}</span>
                                @endforeach
                            @endif
                        </div>
                    </div>
                        ...
                        <div class="bim-register-card" id="bim-register">
                            ...
                            <form class="bim-form-grid" id="bimTrainingForm" novalidate>
                                ...
                                <div class="bim-radio-row">
                                    <label><input type="radio" name="bimMode" value="Offline" checked> Offline Training
                                        (Delhi)</label>
                                </div>
                ...
                <div class="bim-cert-section">
                    <div class="bim-cert-section-head">
                        <h2>{{ $content->certificate_section_title ?? 'Program Certificate' }}</h2>
                        <p>{{ $content->certificate_section_desc ?? 'Every student who completes the BIM Professional Training Program receives an official Eshan Buildwell certificate recognising their skills in 3D, 4D, and 5D BIM workflow.' }}</p>
                    </div>
                    ...
                    <div class="bim-cert-outer">
                        ...
                        <div class="bim-cert-topbar">
                            ...
                            <span class="org-name">{{ $content->certificate_org_name ?? 'ESHAN BUILDWELL INDIA' }}</span>
                        </div>
                        ...
                        <div class="bim-cert-inner">
                            ...
                            <div class="bim-cert-title">{{ $content->certificate_title ?? 'Certificate of Completion' }}</div>
                            <div class="bim-cert-awarded">{{ $content->certificate_awarded_to_text ?? 'has been awarded to' }}</div>
                            <div class="bim-cert-name">Er. Your Name Here</div>

                            <hr class="bim-cert-divider">

                            <div class="bim-cert-body-text">
                                {!! $content->certificate_body_text ?? 'Has successfully completed the <strong>BIM Professional Training Program</strong> conducted by Eshan Buildwell India with dedication, hard work, and outstanding performance in all practical sessions and project-based assignments.' !!}
                            </div>

                            <div class="bim-cert-course-box">
                                <strong>{{ $content->certificate_course_title ?? 'BIM Professional Training Program' }}</strong>
                                {{ $content->certificate_course_tools ?? 'Architecture & Structure (3D) · Planning (4D) · Costing (5D)' }}<br>
                                <span style="font-size:10px; color:#888;">Tools: Autodesk Revit &nbsp;·&nbsp; Navisworks
                                    &nbsp;·&nbsp; DiRoots Plugins</span>
                            </div>
                            ...
                            <div class="bim-cert-footer-row">
                                <div class="bim-cert-sig">
                                    <div class="bim-cert-sig-name">{{ $content->certificate_signature_name ?? 'Gagandeep' }}</div>
                                    <div class="bim-cert-sig-line"></div>
                                    <div class="bim-cert-sig-role">
                                        {!! $content->certificate_signature_role ?? 'Er. Gagandeep Rajput<br>Founder & Director<br>Eshan Buildwell India' !!}
                                    </div>
                                </div>
